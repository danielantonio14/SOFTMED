...
if (isset($_GET['view_pdf']) && $_GET['view_pdf'] == 'funcionarios') {
  ...
  $query = $BD->query("SELECT *FROM funcionarios 
    ...
    INNER JOIN localidades ON funcionarios.id_localidade = localidades.idlocalidades
  ...
  while ($funcionario = $query->fetch()) :
    ...
    $thead2 .= '<td>' . $funcionario->designacao_localidade . '</td>';
  ...
  <body class="fundo">
    ...
    <h1>SOFTMED</h1>
    <h2>RELATÓRIO DE TODOS OS FUNCIONÁRIOS NO SISTEMA.<br> <b>total de registros(' . $contar . ')</b> </h2>
